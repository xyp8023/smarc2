...
class GeoConverterService:
    ...
    def convert(request:UTMLatLon.Request, response:UTMLatLon.Response):
        ...
        for ll in request.lat_lon_points:
            # each is a GeoPoint aleady, so we can feed directly to geodesy
        ...
        for utm in request.utm_points:
            ...
            utm_pt.band = band
            # dont wanna crash if 1 of 100 points was bad...
            try:
                msg = utm_pt.toMsg()
            except:
                msg = None
...
def test_converter():
    ...
    for i in range(len(res.lat_lon_points)):
        ...
        print(f"in:{ll_in}, out:{ll_out}, diff: {diffs}")
        ...
        print(f"in: {utm_in}, out: {utm_out}, diff: {diffs}")